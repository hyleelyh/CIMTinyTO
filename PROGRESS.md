...
## Last Execution Run: 2026-09-14 20:52
### [Built & Documented]
...
- **Microarchitectural Foundations & Theoretical Formalization:**
  - **End-to-End Data Precision Lifecycle:** Fully formalized FPGA $\leftrightarrow$ ASIC data transformation across ResNet image recognition (unfolded `uint8` 255$ with scale  = 1/255 \to$ 1-bit temporal Bernoulli stream $\to$ 2-bit tri-state step $\{-1, 0, +1\} \to$ 5-bit spatial Wallace delta 16 \to$ 13-bit signed accumulator 4095 \to$ 16-bit serialized readback $\to$ 32-bit BRAM multi-block partial sum accumulation $\to$ re-quantized `uint8`).
  ...
  - **LFSR Stream Determinism & Randomness:** Formulated the mathematical proof demonstrating that the Galois LFSR comparator output for =180$ is 100% deterministic (Hamiltonian cycle visits every integer ..255$ exactly once, guaranteeing identically 180 ones over 256 cycles with 0% variance).
  - **The Correlation / Squaring Problem ( \times A \ne A^2$):** Analyzed the Boolean idempotency trap ( \land x = x$) and proved that stride-15 spatial phase separation ensures $|r_{ij}| \le 0.0259 \ll 0.05$, preventing squaring distortion and guaranteeing true multiplication linearity across all 16 channels.
  - **Monte Carlo Precision Scaling vs. Full-Period Collapse:** Formulated the Central Limit Theorem error scaling ($\text{RMSE} \propto 1/\sqrt{N}$) across intermediate truncations (=16, 32, 64, 128$) and validated full-period collapse at =256$ ($\text{RMSE} = 0.00057$, $\text{SNR} = 48.03\text{ dB}$).
  ...
  - **Dynamic Power & Sparsity Benchmarking (`sparsity_pct`):** Formalized the physical mapping of test vector `sparsity_pct` to CMOS dynamic power  = \alpha C V^2 f$, establishing the post-silicon current benchmark ($\sim 2.5\text{ mA}$ at 5.66\%$ sparsity vs. $\sim 10\text{ mA}$ at bash\%$ sparsity).
